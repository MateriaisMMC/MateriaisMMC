function [x,est] = metJacobi(A,b,tol,maxit,x0,impr)
%metJacobi Resolução de um sistema linear pelo método iterativo de Jacobi
%
% [x,est] = metJacobi(A,b,tol,maxit,x0,impr)   
%   Determinação da solução aproximada do sistema Ax=b e de uma estimativa 
%   para o respetivo erro, usando o método iterativo de Jacobi.
%
...
%
%   O processo é inicializado com o vetor x0 
%   e, se impr = 1, os resultados intermédios são visualizados.
%----------------------------------------------------------------------
% 
% INPUT:
...
%       impr: usar impr = 1, se pretendermos ver resultados intermédios
%             Por defeito, impr = 0
%          
% OUTPUT:
...
%          (norma infinita da diferença das duas últimas iteradas)     
%
...
%------------------------------------------------------------------------
dA = diag(A);
A = diag(dA)-A;	%  Formação da matriz -(M+N) e sua sobreposição 
                %   à matriz A                     
niter = 0;  % Inicialização do contador do número de iterações
while  niter < maxit 
    x = (A*x0+b)./dA;       % Cálculo da nova iterada
 ...
 % --------------------------------------------------------------------
    if est < tol
       disp(' ')
       disp(['O Método de Jacobi atingiu a precisão desejada em ',num2str(niter), ' iterações'])
       break
    end
...
if (niter == maxit && (est >= tol||isnan(est)))
   disp(' ')
   disp(['Ao fim de ',num2str(maxit), ' iterações do Método de Jacobi,não se atingiu a precisão desejada'])
end